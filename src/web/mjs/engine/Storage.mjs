...
export default class Storage {
    ...
    saveChapterNovel(chapter, content) {
        try {
            let pageData = [{
                name: this._pageFileName(1, Engine.Settings.novelFormat.value),
                data: content.html
            }];
            let promise = undefined;
            let output = this._chapterOutputPath(chapter);
            if (Engine.Settings.novelFormat.value === 'text/plain') {
                this._createDirectoryChain(output);
                promise = this._saveChapterPagesFoldertext(output, pageData)
                    .then(() => this._runPostChapterDownloadCommand(chapter, output));
            }
            if (Engine.Settings.novelFormat.value === 'text/html') {
                this._createDirectoryChain(output);
                promise = this._saveChapterPagesFolderhtml(output, pageData)
                    .then(() => this._runPostChapterDownloadCommand(chapter, output));
            }
            return promise || Promise.reject(new Error('Unsupported output format: ' + Engine.Settings.chapterFormat.value));
        } catch (error) {
    ...
    _saveChapterPagesFolderhtml(directory, pageData) {
        let promises = pageData.map(page => {
            return this._writeFile(this.path.join(directory, page.name), page.data);
        });
        return Promise.all(promises);
    }

    _saveChapterPagesFoldertext(directory, pageData) {
        let promises = pageData.map(page => {
            return this._writeFile(this.path.join(directory, page.name), this.html2text(page.data));
        });
        return Promise.all(promises);
    }
